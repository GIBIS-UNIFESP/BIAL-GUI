...
SegmentationTool::SegmentationTool( GuiImage *guiImage, ImageViewer *viewer ) : Tool( guiImage,
                                                                                      viewer ){
    if(guiImage->getImageType() == image_type::type_color){
        seeds = Bial::Image< char >( guiImage->getCImage().Dim() );
    }
    if(guiImage->getImageType() == image_type::type_int){
        seeds = Bial::Image< char >( guiImage->getIImage().Dim() );
    }
    drawType = 1;
    drawing = false;
...
Bial::Image< char > SegmentationTool::segmentationOGS( double alpha, double beta ) {
    ...
    if( ( !img.empty( ) ) && ( !bkg.empty( ) ) ) {
        ...
        //TODO: Considerar imagens float
        if(guiImage->getImageType() == image_type::type_int){
            res = Bial::Segmentation::OrientedGeodesicStar( guiImage->getIImage( ), img, bkg, alpha, beta );
        }

        else{
            throw std::runtime_error(" Color image not suported ");
        }
